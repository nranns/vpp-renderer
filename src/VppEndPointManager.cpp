...
namespace VPP
{
EndPointManager::EndPointManager(opflexagent::Agent &agent, IdGen &id_gen,
                                 Uplink &uplink,
...
std::shared_ptr<interface> EndPointManager::mk_bd_interface(
    const opflexagent::Endpoint &ep, const bridge_domain &bd,
    const route_domain &rd) throw(EndPointManager::NoEpInterface)
{
    ...
    {
        uint16_t vlan_id;
        interface intf(iname, getIntfTypeFromName(iname),
                       interface::admin_state_t::UP, uuid);
        OM::write(uuid, intf);
    ...
    {
        interface intf(iname, getIntfTypeFromName(iname),
                       interface::admin_state_t::UP, rd, uuid);
        OM::write(uuid, intf);
        itf = intf.singular();
    }
     ...
     */
    if (handle_t::INVALID == itf->handle()) throw;

    return itf;
}

static void allow_dhcp_request(ACL::l3_list::rules_t &in_rules,
...
void EndPointManager::handle_interface_stat_i(
    interface_cmds::stats_enable_cmd *e)
{
    OLOGD << "Interface Stat: " << *e;
    ...
    for (auto &msg : *e)
    {
        ...
        for (int i = 0; i < payload.count; i++)
        {
            ...
            OLOGD << "Interface Stat: " << sp->to_string()
...
void EndPointManager::handle_interface_stat(interface_cmds::stats_enable_cmd *e)
{
    m_agent.getAgentIOService().dispatch(
        std::bind(&EndPointManager::handle_interface_stat_i, this, e));
}

void EndPointManager::handle_update(const std::string &uuid)
{
    /*
    ...
    if (!epWrapper)
    {
        OLOGD << "Deleting endpoint " << uuid;
        return;
    }
    OLOGD << "Updating endpoint " << uuid;
    ...
    try
    {
        ...
        try
        {
            ...
            if (v4c)
            {
                ...
                allow_dhcp_request(in_rules, out_rules,
            ...
            SecurityGroupManager::build_update(m_agent, secGrps, secGrpId,
                                               in_rules, out_rules,
                                               ethertype_rules);
            ...
            if (itf->handle().value())
            {
                ...
                {
                    ACL::l2_rule rulev6(50, ACL::action_t::PERMIT,
                                        route::prefix_t::ZEROv6, macAddr,
                                        mac_address_t::ZERO);

                    ACL::l2_rule rulev4(51, ACL::action_t::PERMIT,
                                        route::prefix_t::ZERO, macAddr,
                                        mac_address_t::ZERO);
                    rules.insert(rulev4);
                    rules.insert(rulev6);
                }
                ...
                {
                    ACL::l2_rule rulev6(20, ACL::action_t::PERMIT,
                                        route::prefix_t::ZEROv6, macAddr,
                                        mac_address_t::ONE);

                    ACL::l2_rule rulev4(21, ACL::action_t::PERMIT,
                                        route::prefix_t::ZERO, macAddr,
                                        mac_address_t::ONE);
                    rules.insert(rulev4);
                    rules.insert(rulev6);
                    ...
                    for (auto &ipAddr : ipAddresses)
                    {
                        ...
                        if (ipAddr.is_v6())
                        {
                            ACL::l2_rule rule(30, ACL::action_t::PERMIT, pfx,
                                              macAddr, mac_address_t::ONE);
                            rules.insert(rule);
                        }
                        else
                        {
                            ACL::l2_rule rule(31, ACL::action_t::PERMIT, pfx,
                                              macAddr, mac_address_t::ONE);
                            rules.insert(rule);
                        }
                    }
                     ...
                     ep.getVirtualIPs())
                {
                    ...
                    for (auto &ipAddr : ipAddresses)
                    {
                        ...
                        if (ipAddr.is_v6())
                        {
                            ACL::l2_rule rule(60, ACL::action_t::PERMIT, pfx,
                                              vmac, mac_address_t::ONE);
                            rules.insert(rule);
                        }
                        else
                        {
                            ACL::l2_rule rule(61, ACL::action_t::PERMIT, pfx,
                                              vmac, mac_address_t::ONE);
                            rules.insert(rule);
                        }
                    }
            ...
            interface bvi("bvi-" + std::to_string(bd.id()),
                          interface::type_t::BVI, interface::admin_state_t::UP,
                          rd);
            OM::write(uuid, bvi);
            ...
            if (hasMac)
            {
                ...
                /*
                 * An entry in the BD's L2 FIB to forward traffic to the
                 * end-point
                 */
                bridge_domain_entry be(bd, vmac, *itf);
                OM::write(uuid, be);
                                 ...
                                 epgURI.get())))
                {
                    ...
                    if (0 != ipms.size())
                    {
                        ...
                        nat_binding recirc_nb4(recirc_itf, direction_t::INPUT,
                                               l3_proto_t::IPV4,
                                               nat_binding::zone_t::OUTSIDE);
                        OM::write(uuid, recirc_nb4);

                        nat_binding recirc_nb6(recirc_itf, direction_t::INPUT,
                                               l3_proto_t::IPV6,
                                               nat_binding::zone_t::OUTSIDE);
                        OM::write(uuid, recirc_nb6);

                        gbp_recirc grecirc(recirc_itf,
                                           gbp_recirc::type_t::INTERNAL, gepg);
                        ...
                        for (auto &ipm : ipms)
                        {
                            ...
                            try
                            {
                                ...
                                interface ext_bvi(
                                    "bvi-" + std::to_string(ffwd.bdId),
                                    interface::type_t::BVI,
                                    interface::admin_state_t::UP, ext_rd);
                                 ...
                                 * interface
                                 */
                                bridge_domain_entry fp_be(ext_bd, macAddr,
                                                          recirc_itf);
                                OM::write(uuid, fp_be);
                            ...
                            {
                                OLOGD << "Endpoint Floating IP no fwd " << uuid;
                            }
                        }
                    }
                }
        ...
        catch (EndPointManager::NoEpInterface &noepitf)
        {
            OLOGD << "Endpoint - no interface " << uuid;
        }
    }
    catch (EndPointGroupManager::NoFowardInfo &nofwd)
    {
        OLOGD << "Endpoint - no fwding " << uuid;
    }
